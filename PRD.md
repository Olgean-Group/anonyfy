...
2. **Le dictionnaire de code.** Un mapping déterministe *est* un code book. Qui obtient
   ...
   par déploiement, sel par scope. La rotation de clé est reportée à **v2** : en v1,
   elle exigerait un registre stockant la valeur claire pour rejouer les HMAC et
   re-chiffrer les substituts après K→K', ce qui contredirait l'invariant 1 (« le
   clair ne franchit jamais la frontière ») et la promesse « jamais de clair
   stocké ». La v2 introduirait un registre chiffrant la valeur claire sous KMS,
   avec consentement explicite du client. Voir ADR 0001 (hors périmètre v1).
